...
import { getOnePostBSlug, getRecommendPosts } from '@/lib/getPosts';
import { notFound } from 'next/navigation';
import PreviewPost from '@/components/PreviewPost';

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
    const post = getOnePostBSlug((await params).slug);
    if (!post) {
        notFound();
    }
    const recommendPosts = getRecommendPosts(post.category.link, post.id);
        ...
        <>
            ...
            <figure className="mb-12 border-2 border-cyan-400/30 rounded-lg p-2 glass-effect">
                <Image
                    ...
                    className="w-full h-auto object-cover rounded-md animate-glow-breathe"
                />
            </figure>
