...
export default function PreviewPost({ post, colorKey }: { post: PostType; colorKey: keyof typeof colorClasses }) {
	...
	return (
		<div
			...
			<Image
				...
				alt={`Imagem do post sobre ${post.category}`}
				className="w-full h-48 object-cover"
			/>
			<div className="p-6 flex flex-col flex-grow">
				<p className={`font-chakra text-glow-pink text-xl mb-2 ${color.text}`}>{post.category.name.toUpperCase()}</p>
				...
				<div className="flex flex-wrap gap-2 mb-6">
					{post.tags.map((tag) => (
						<span
							key={tag}
							className={`${color.bg + ' ' + color.textBtn} text-xs font-medium px-2.5 py-1 rounded-full}`}>
							{tag}
						</span>
					))}
				</div>
